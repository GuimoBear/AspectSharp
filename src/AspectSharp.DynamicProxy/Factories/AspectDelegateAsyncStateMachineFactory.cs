...
namespace AspectSharp.DynamicProxy.Factories
{
    internal static class AspectDelegateAsyncStateMachineFactory
    {
        internal static GeneratedAsyncStateMachine GenerateAsyncStateMachine(ModuleBuilder moduleBuilder, MethodInfo methodInfo, MethodBuilder callerMethod, GenericTypeParameterBuilder[] callerMethodGenericParameters)
        {
            ...
            var builderField = DefineMethodBuilderField(typeBuilder, callerMethod, genericParameterBuilders);
            var contextField = DefineContextField(typeBuilder);
            var awaiterField = DefineAwaiterField(typeBuilder, methodToCall, genericParameterBuilders);
            ...
            DefineMoveNext(typeBuilder, methodToCall, methodParameters, stateField, builderField, contextField, contextWrapField, awaiterField, genericParameterBuilders);
        ...
        private static FieldBuilder DefineAwaiterField(TypeBuilder typeBuilder, MethodInfo methodInfo, GenericTypeParameterBuilder[] genericParameters)
            => typeBuilder.DefineField("<>__awaiter", GetTaskAwaiterType(GenericParameterUtils.ReplaceTypeUsingGenericParameters(methodInfo.ReturnType, genericParameters)), FieldAttributes.Private);

        private static MethodBuilder DefineMoveNext(TypeBuilder typeBuilder, MethodInfo methodInfo, ParameterInfo[] methodParameters, FieldBuilder stateField, FieldBuilder builderField, FieldBuilder contextField, FieldBuilder contextWrapField, FieldBuilder awaiterField, GenericTypeParameterBuilder[] genericParameters)
        {
            var prepareAwaiterMethod = DefinePrepareAwaiter(typeBuilder, methodInfo, methodParameters, contextField, contextWrapField, genericParameters);
            var awaitOnCompletedMethod = DefineAwaitOnCompleted(typeBuilder, methodInfo, stateField, awaiterField, builderField, genericParameters);
            var afterCompletionMethod = DefineAfterCompletionMethod(typeBuilder, methodInfo, contextWrapField, genericParameters);

            var awaiterType = GetTaskAwaiterType(GenericParameterUtils.ReplaceTypeUsingGenericParameters(methodInfo.ReturnType, genericParameters));
        ...
        private static MethodBuilder DefinePrepareAwaiter(TypeBuilder typeBuilder, MethodInfo methodInfo, ParameterInfo[] methodParameters, FieldBuilder contextField, FieldBuilder contextWrapField, GenericTypeParameterBuilder[] genericParameters)
        {
            var returnInfo = methodInfo.GetReturnInfo();

            var taskAwaiterType = GetTaskAwaiterType(GenericParameterUtils.ReplaceTypeUsingGenericParameters(methodInfo.ReturnType, genericParameters));
            ...
            if (hasParameters)
                foreach (var parameter in methodParameters)
                    parameters.Add(cil.DeclareLocal(GenericParameterUtils.ReplaceTypeUsingGenericParameters(parameter.ParameterType, genericParameters)));
            LocalBuilder localValueTask = default;
#if NETCOREAPP3_1_OR_GREATER
        if (returnInfo.IsValueTask)
            localValueTask = cil.DeclareLocal(GenericParameterUtils.ReplaceTypeUsingGenericParameters(methodInfo.ReturnType, genericParameters));
#endif
            ...
            if (returnInfo.IsValueTask)
            {
                ...
                cil.Emit(OpCodes.Call, methodInfo.ReturnType.ContainsGenericParameters
                    ? TypeBuilder.GetMethod(GenericParameterUtils.ReplaceTypeUsingGenericParameters(methodInfo.ReturnType, genericParameters), typeof(ValueTask<>).GetMethod(nameof(Task.GetAwaiter), Type.EmptyTypes))
                    : GenericParameterUtils.ReplaceTypeUsingGenericParameters(methodInfo.ReturnType, genericParameters).GetMethod(nameof(Task.GetAwaiter), Type.EmptyTypes));
            }
            else
#endif
                cil.Emit(OpCodes.Callvirt, methodInfo.ReturnType.ContainsGenericParameters 
                    ? TypeBuilder.GetMethod(GenericParameterUtils.ReplaceTypeUsingGenericParameters(methodInfo.ReturnType, genericParameters), typeof(Task<>).GetMethod(nameof(Task.GetAwaiter), Type.EmptyTypes))
                    : GenericParameterUtils.ReplaceTypeUsingGenericParameters(methodInfo.ReturnType, genericParameters).GetMethod(nameof(Task.GetAwaiter), Type.EmptyTypes));
            cil.Emit(OpCodes.Ret);
            return methodBuilder;
        }

        private static MethodBuilder DefineAwaitOnCompleted(TypeBuilder typeBuilder, MethodInfo methodInfo, FieldBuilder stateField, FieldBuilder awaiterField, FieldBuilder builderField, GenericTypeParameterBuilder[] genericParameters)
        {
            var taskAwaiterType = GetTaskAwaiterType(GenericParameterUtils.ReplaceTypeUsingGenericParameters(methodInfo.ReturnType, genericParameters));
        ...
        private static MethodBuilder DefineAfterCompletionMethod(TypeBuilder typeBuilder, MethodInfo methodInfo, FieldBuilder contextWrapField, GenericTypeParameterBuilder[] genericParameters)
        {
            var taskAwaiterType = GetTaskAwaiterType(GenericParameterUtils.ReplaceTypeUsingGenericParameters(methodInfo.ReturnType, genericParameters));
            var taskAwaiterByRefType = taskAwaiterType.MakeByRefType();
            ...
            if (methodInfo.ReturnType.IsGenericType)
                localResult = cil.DeclareLocal(GenericParameterUtils.ReplaceTypeUsingGenericParameters(methodInfo.ReturnType, genericParameters).GetGenericArguments()[0]);
            ...
            if (methodInfo.ReturnType.IsGenericType)
            {
                var parameterType = GenericParameterUtils.ReplaceTypeUsingGenericParameters(methodInfo.ReturnType, genericParameters).GetGenericArguments()[0];
        ...
        private static MethodInfo GetCreateMethodBuilderMethod(FieldBuilder builderField, GenericTypeParameterBuilder[] genericParameters)
            => GenericParameterUtils.ReplaceTypeUsingGenericParameters(builderField.FieldType, genericParameters).GetMethod(nameof(AsyncTaskMethodBuilder.Create));

        private static MethodInfo GetStartMethodBuilderGenericMethod(FieldBuilder builderField, Type stateMachineType, GenericTypeParameterBuilder[] genericParameters)
            => GenericParameterUtils.ReplaceTypeUsingGenericParameters(builderField.FieldType, genericParameters).GetMethod(nameof(AsyncTaskMethodBuilder.Start)).MakeGenericMethod(stateMachineType);

        private static MethodInfo GetTaskGetterOnAsyncMethodBuilderMethod(FieldBuilder builderField, GenericTypeParameterBuilder[] genericParameters)
            => GenericParameterUtils.ReplaceTypeUsingGenericParameters(builderField.FieldType, genericParameters).GetProperty(nameof(AsyncTaskMethodBuilder.Task)).GetGetMethod();
        ...
        internal class GeneratedAsyncStateMachine
        {
            ...
            public GeneratedAsyncStateMachine(MethodBuilder callerMethod, GenericTypeParameterBuilder[] callerMethodGenericParameters, Type type, FieldBuilder stateField, FieldBuilder builderField, FieldBuilder contextField)
            {
            ...
            public void WriteCallerMethod()
            {
                ...
                var localAwaiterStateMachine = cil.DeclareLocal(GenericParameterUtils.ReplaceTypeUsingGenericParameters(Type, _callerMethodGenericParameters));

                cil.Emit(OpCodes.Ldloca_S, localAwaiterStateMachine.LocalIndex);
                cil.Emit(OpCodes.Call, GetCreateMethodBuilderMethod(BuilderField, _callerMethodGenericParameters));
                ...
                cil.Emit(OpCodes.Ldflda, BuilderField);
                cil.Emit(OpCodes.Ldloca_S, localAwaiterStateMachine.LocalIndex);
                cil.Emit(OpCodes.Call, GetStartMethodBuilderGenericMethod(BuilderField, GenericParameterUtils.ReplaceTypeUsingGenericParameters(Type, _callerMethodGenericParameters), _callerMethodGenericParameters));

                cil.Emit(OpCodes.Ldloca_S, localAwaiterStateMachine.LocalIndex);
                cil.Emit(OpCodes.Ldflda, BuilderField);
                cil.Emit(OpCodes.Call, GetTaskGetterOnAsyncMethodBuilderMethod(BuilderField, _callerMethodGenericParameters));
                cil.Emit(OpCodes.Ret);
            }
        }
