...
namespace AspectSharp.Tests.Net9.DynamicProxy
{
    ...
    {
        public PipelineExecutionTests(SaveFixtures _) { }
        ...
        [Fact]
        public async Task Given_AnService_When_ExecuteValueAsyncFunction_Then_PipelineExecutionShouldBeFullyExecuted()
        {
            var serviceType = typeof(IService);
            var targetType = typeof(Service);
            var configs = new DynamicProxyFactoryConfigurations(InterceptedEventMethod.None, InterceptedPropertyMethod.None, false, default);
